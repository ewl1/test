...
function can_write($file){ return file_exists($file) ? is_writable($file) : is_writable(dirname($file)); }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['save_config'])) {
        ...
        if (!can_write($root . '/config.php')) $error = 'Nepavyko įrašyti config.php';
        else { file_put_contents($root . '/config.php', $content); header('Location: install.php?step=db'); exit; }
    }
    ...
    if (isset($_POST['create_admin'])) {
        ...
        try {
            $pdo = new PDO('mysql:host=' . DB_HOST . ';dbname=' . DB_NAME . ';charset=utf8mb4', DB_USER, DB_PASS, [PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION]);
            $stmt = $pdo->prepare("INSERT INTO users (username, email, password, role_id, is_active, status, created_at) VALUES (:u,:e,:p,1,1,'active',NOW())");
            $stmt->execute([':u'=>trim($_POST['username']), ':e'=>trim($_POST['email']), ':p'=>password_hash((string)$_POST['password'], PASSWORD_DEFAULT)]);
            $success = 'Admin sukurtas.';
            $step = 'done';
        } catch (Throwable $e) { $error = $e->getMessage(); }
    }
}
?><!doctype html><html lang="lt"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Install</title><link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css"></head>
<body class="bg-body-tertiary"><div class="container py-5"><div class="mx-auto" style="max-width:820px"><div class="card shadow-sm"><div class="card-body p-4">
...
<?php elseif ($step === 'db'): ?>
<form method="post"><button class="btn btn-primary" name="run_sql" value="1">Importuoti DB</button></form>
...
<?php else: ?><div class="alert alert-success">Baigta. Prisijunk per /administration/login.php</div><?php endif; ?>
