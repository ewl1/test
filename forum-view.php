    ...
    <div class="col-xl-10">
        ...
        <div class="card forum-forum-header-card mb-4">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-start gap-3 flex-wrap">
                    ...
                    <a class="btn btn-outline-secondary" href="<?= forum_index_url() ?>">Visi forumai</a>
                </div>
            </div>
        </div>
        ...
        <div class="card mb-4">
            ...
            <div class="card-body p-0">
                <?php if (!$topics): ?>
                    <div class="p-4 text-secondary">Siame forume temu dar nera.</div>
                <?php else: ?>
                    <?php foreach ($topics as $topic): ?>
                        <article class="forum-topic-list-item">
                            ...
                            <div class="forum-topic-stats">
                                <div><strong><?= (int)$topic['reply_count'] ?></strong><span>Atsakymai</span></div>
                                <div><strong><?= (int)$topic['views'] ?></strong><span>Perziuros</span></div>
                            </div>
                            <div class="forum-topic-last">
                                <div class="small text-secondary mb-1">Paskutinis atsakymas</div>
                                <div class="fw-semibold">
                                    <?php if (!empty($topic['last_post_user_id'])): ?>
                                        <a class="text-decoration-none" href="<?= user_profile_url((int)$topic['last_post_user_id']) ?>">
                                            <?= e($topic['last_post_username'] ?? 'Narys') ?>
                                        </a>
                                    <?php else: ?>
                                        <?= e($topic['last_post_username'] ?? 'Svečias') ?>
                                    <?php endif; ?>
                                </div>
        ...
        <div class="card forum-editor-card">
            <div class="card-header">Kurti nauja tema</div>
            <div class="card-body">
                <?php if ($formError): ?>
                ...
                <?php if (!current_user()): ?>
                    <div class="alert alert-info mb-0">Temas gali kurti tik prisijunge nariai. <a href="<?= public_path('login.php') ?>">Prisijunkite</a>.</div>
                <?php else: ?>
                    <form method="post">
                        <?= csrf_field() ?>
                        ...
                        <div class="mb-3">
                            <label class="form-label" for="forum-topic-title">Temos pavadinimas</label>
                            <input class="form-control" id="forum-topic-title" name="title" maxlength="190" value="<?= e($topicTitle) ?>" required>
                        </div>

                        <div class="mb-3">
                            <label class="form-label" for="forum-topic-content">Zinute</label>
                        ...
                        <button class="btn btn-primary">Sukurti tema</button>
                    </form>
                <?php endif; ?>
            </div>
        </div>
