...
<div class="row justify-content-center">
    <div class="col-xl-10">
            ...
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-start gap-3 flex-wrap">
                    ...
                    <div class="forum-hero-actions">
                        <a class="btn btn-outline-primary" href="<?= forum_index_url() ?>">Atnaujinti</a>
                    </div>
                </div>
            </div>
        </div>
        ...
        <?php if (!$categories): ?>
            <div class="alert alert-info">Forumo kategoriju dar nera. Sukurkite jas per administravimo puse.</div>
        <?php endif; ?>
            ...
            <section class="card forum-category-card mb-4">
                ...
                <div class="card-body p-0">
                    <?php if (!$category['forums']): ?>
                        <div class="p-4 text-secondary">Sioje kategorijoje forumu dar nera.</div>
                    <?php else: ?>
                        <?php foreach ($category['forums'] as $forum): ?>
                            <article class="forum-forum-row">
                                <div class="forum-forum-main">
                                    ...
                                    <?php if (!empty($forum['subforums'])): ?>
                                        <div class="forum-subforum-list">
                                            <span class="forum-subforum-label">Subforumai:</span>
                                ...
                                <div class="forum-forum-stats">
                                    <div><strong><?= (int)$forum['topics_count'] ?></strong><span>Temos</span></div>
                                    <div><strong><?= (int)$forum['posts_count'] ?></strong><span>Pranesimai</span></div>
                                </div>

                                <div class="forum-forum-latest">
                                    <?php if (!empty($forum['last_topic_id'])): ?>
                                        <div class="small text-secondary mb-1">Paskutinis aktyvumas</div>
                                        ...
                                        <div class="small text-secondary">
                                            <?php if (!empty($forum['last_post_user_id'])): ?>
                                                <a class="text-decoration-none" href="<?= user_profile_url((int)$forum['last_post_user_id']) ?>">
                                                    <?= e($forum['last_post_username'] ?: 'Narys') ?>
                                                </a>
                                            <?php else: ?>
                                                <?= e($forum['last_post_username'] ?: 'Svečias') ?>
                                            <?php endif; ?>
                                            · <?= e(format_dt($forum['last_post_at'])) ?>
                                        </div>
                                    <?php else: ?>
                                        <div class="small text-secondary">Kol kas jokio aktyvumo.</div>
                                    <?php endif; ?>
                                </div>
                            </article>
                        <?php endforeach; ?>
