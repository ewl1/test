...
$settingsTab = isset($_GET['tab']) ? trim((string)$_GET['tab']) : 'general';
$allowedTabs = ['general', 'display', 'uploads', 'security'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ([
        ...
        'image_upload_max_size_kb',
        'image_upload_max_width',
        'image_upload_max_height',
        'image_upload_quota_per_user_mb',
    ...
    redirect('settings.php?tab=' . urlencode($settingsTab));
}
...
admin_render_page_header([
    ...
    'subtitle' => 'Branduolio, rodymo, upload ir saugumo nustatymai vienoje vietoje',
    'actions' => [
        [
...
<div class="col-lg-8 admin-layout-main">
    <ul class="nav nav-tabs mb-4" role="tablist">
        <li class="nav-item" role="presentation">
            <a class="nav-link <?= $settingsTab === 'general' ? 'active' : '' ?>" href="settings.php?tab=general">Pagrindas</a>
        </li>
        <li class="nav-item" role="presentation">
            <a class="nav-link <?= $settingsTab === 'display' ? 'active' : '' ?>" href="settings.php?tab=display">Rodymas</a>
        </li>
        <li class="nav-item" role="presentation">
            <a class="nav-link <?= $settingsTab === 'uploads' ? 'active' : '' ?>" href="settings.php?tab=uploads">Paveiksliukai</a>
        </li>
        <li class="nav-item" role="presentation">
            <a class="nav-link <?= $settingsTab === 'security' ? 'active' : '' ?>" href="settings.php?tab=security">Saugumas</a>
        </li>
    </ul>
    ...
    <?php if ($settingsTab === 'uploads'): ?>
        <div class="card mb-4">
            <div class="card-header">Paveiksliuku upload taisykles</div>
            <div class="card-body row g-3">
                <div class="col-md-6">
                    <label class="form-label">Paveiksliuku ikelimas</label>
                    <select class="form-select" name="image_uploads_enabled">
                        <option value="1" <?= setting('image_uploads_enabled', '1') === '1' ? 'selected' : '' ?>>Ijungtas</option>
                        <option value="0" <?= setting('image_uploads_enabled', '1') === '0' ? 'selected' : '' ?>>Isjungtas</option>
                    </select>
                </div>
                <div class="col-md-6">
                    <label class="form-label">Maksimalus dydis KB</label>
                    <input class="form-control" type="number" min="64" max="51200" name="image_upload_max_size_kb" value="<?= e(setting('image_upload_max_size_kb', '2048')) ?>">
                </div>
                <div class="col-md-6">
                    <label class="form-label">Maksimalus plotis px</label>
                    <input class="form-control" type="number" min="64" max="12000" name="image_upload_max_width" value="<?= e(setting('image_upload_max_width', '4096')) ?>">
                </div>
                <div class="col-md-6">
                    <label class="form-label">Maksimalus aukstis px</label>
                    <input class="form-control" type="number" min="64" max="12000" name="image_upload_max_height" value="<?= e(setting('image_upload_max_height', '4096')) ?>">
                </div>
                <div class="col-md-6">
                    <label class="form-label">Vartotojo kvota MB</label>
                    <input class="form-control" type="number" min="0" max="10240" name="image_upload_quota_per_user_mb" value="<?= e(setting('image_upload_quota_per_user_mb', '25')) ?>">
                    <div class="form-text">0 reiskia be kvotos limito.</div>
                </div>
                <div class="col-12">
                    <label class="form-label">Leidziami MIME tipai</label>
                    <textarea class="form-control" name="image_upload_allowed_mime" rows="3" placeholder="image/jpeg,image/png,image/gif,image/webp"><?= e(setting('image_upload_allowed_mime', 'image/jpeg,image/png,image/gif,image/webp')) ?></textarea>
                    <div class="form-text">Naudokite kableliu arba tarpu atskirta sarasa.</div>
                </div>
            </div>
        </div>
    <?php endif; ?>

    <?php if ($settingsTab === 'security'): ?>
        <div class="card mb-4">
